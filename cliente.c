...
void bajaCliente(char archivo[], int id){//RECIBE POR PARAMETRO NUESTRO ARCHIVO DE CLIENTES
    FILE *archi = fopen(archivo,"r+b");//Abrimos el archivo en modo lectura y escritura
    stCliente a;
    int flag = 0;
    int opcion;
    if(archi){
        while(fread(&a, sizeof(stCliente), 1, archi) > 0 && flag == 0){
            if(a.id == id){
                ...
                if(opcion == 83 || opcion == 115){
                    ...
                    fclose(archi);
                    printf("\n El cliente ha cambiado de estado\n");
                    muestraUnCliente(a);
                    printf("\n");
                    system("pause");
                    system("cls");
